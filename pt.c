...
static uint32_t crc32buf(char *buf, size_t len)
{
	uint32_t crc;
...
static int check_part(FILE *fp, uint32_t crc, int offset, size_t len)
{
	char *buf;
	uint32_t tmp_crc = 0;
	int ret = 0;
	...
	fread(buf, 1, len, fp);
	fseek(fp, 0, SEEK_SET);
	tmp_crc = crc32buf(buf, len);
	free(buf);

#ifdef MY_DEBUG
	printf("%s: crc = 0x%08X, tmp_crc = 0x%08X, addr = 0x%08X, size = %ld\n", __func__, crc, tmp_crc, offset, (long int)len);
#endif
...
static int fw_table_read(unsigned char *data, int len)
{
	int i;
	unsigned char *part;
...
int check_valid_fw(FILE *fp)
{
	int i, valid = 0;
	char pbuf[FLASH_PARTITION_SIZE] = { 0, };

	start_addr = 0;
	total_size = 0;

	fseek(fp, FLASH_PARTITION_ADDR, SEEK_SET);
	fread(pbuf, 1, FLASH_PARTITION_SIZE, fp);
	fseek(fp, 0, SEEK_SET);

	tbl = (struct partition *)malloc(sizeof(struct partition));
	if(!tbl)
		return -1;

	if(fw_table_read((unsigned char *)pbuf, FLASH_PARTITION_SIZE)) {
		free(tbl);
		return -1;
	}

	if(tbl->magic != PARTITION_MAGIC) {
		free(tbl);
		return -1;
	}

	for (i = 0; i < tbl->count; i++) {
		if(!strncmp(tbl->tables[i].name, PARTITION_TABLE, strlen(PARTITION_TABLE))) {
			start_addr = tbl->tables[i].start_addr;
			total_size += tbl->tables[i].total_size;
		} else if(!strncmp(tbl->tables[i].name, PARTITION_KERNEL, strlen(PARTITION_KERNEL))) {
			if(check_part(fp, tbl->tables[i].crc_verify, tbl->tables[i].start_addr, tbl->tables[i].used_size)) {
				total_size += tbl->tables[i].total_size;
				valid++;
			} else
				total_size = 0;
		} else if(!strncmp(tbl->tables[i].name, PARTITION_ROOT, strlen(PARTITION_ROOT))) {
			if(check_part(fp, tbl->tables[i].crc_verify, tbl->tables[i].start_addr, tbl->tables[i].used_size)) {
				total_size += tbl->tables[i].total_size;
				valid++;
			} else
